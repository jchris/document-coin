...
class Coin {
    ...
    foldBlockNodes(callback) {
      return this.foldRootNode().then((results) => {
    ...
    foldRootNode() {
      ...
      return this.decodeBlock(block, holderKey).then((decoded) => {
        ...
        console.log("folded BLOCK", rootPayload)
  ...
  // TODO validate givetree
  validate() {
    return this.validateRoot().then(()=>{
      return this.foldBlockNodes((decodedBlock, rawBlock, children) => {
        console.log("folded BlockNodes", decodedBlock[1].kid, children.length, decodedBlock[0])
      })
    }).then(()=>{
      return true;
    }).catch((e)=>{
